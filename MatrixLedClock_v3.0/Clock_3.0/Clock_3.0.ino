...
void loop() {
 ...
 Serial.println(MenuOption); //-1 is a small fix to adjust the return value from the font selection menu to the actual clock mode value;
...
byte firstrun = 1;
  ...
  for (int count = 0; count < 35 ; count++) {
    if (buttonA.uniquePress() || firstrun == 1) {
      ...
      if (next_MenuOption >  NumberOfMenuOptions - 1 ) {
        next_MenuOption = 0;
      }
      byte i = 0;
      while (str_top[i]) {
