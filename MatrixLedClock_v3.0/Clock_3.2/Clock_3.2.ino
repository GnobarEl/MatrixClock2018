...
void loop() {
  ...
  Serial.print("Debug: Main LOOP | Clock Mode: ");
...
if (clock_mode > 6) //Guardar o old_clockmod para nao fazer reset caso se seleccione o Exit do menu fonts
{
  clock_mode = 1;
}
  ...
  Serial.print("Debug: Main LOOP - Menu Option: ");
  ...
  if (MainMenuOption >= 0) {
    switch (MenuOption) {
      case 0:
        MenuOption = 10;
        Serial.println("Go to Fonts");
        SelectFonts();
        break;
      case 1:
        MenuOption = 10;
        Serial.println("Go to Set Alarm");
        SetAlarm();
        break;
      case 2:
        MenuOption = 10;
        Serial.println("Go to Set Clock");
        SetTime();
        break;
      case 3:
        MenuOption = 10;
        Serial.println("Go to Bright");
        SetBright();
        break;
      case 4:
...
void SuperBigFont() {
  ...
  while (run_mode()) {
    ...
    //check for button press
    if (buttonA.uniquePress()) {
      Serial.println("Debug: Super Big Font : Menu pressed");
      SettingsMenu();
      return;
    }
    if (buttonB.uniquePress()) {
...
void SettingsMenu() {
  ...
  const char *MenuOptions[] = {"Fonts", "Alarm", "Clock", "Bright", "Exit"};
  int NumberOfMenuOptions = sizeof(MenuOptions) / sizeof(int);
  ...
  for (int count = 0; count < 35 ; count++) {
    if (buttonA.uniquePress() || firstrun == 1) {
          if (MenuOption == NumberOfMenuOptions - 1) {
             MenuOption = 0;
          } else {
             MenuOption++;
          }     
      Serial.print("Selected Option: ");
      Serial.println(MenuOptions[MenuOption]);
      count = 0;
      cls();
      ...
      //Serial.print("Debug: UP with option: ");
      //Serial.println(MenuOption);
    }
    delay(50);
  } 
